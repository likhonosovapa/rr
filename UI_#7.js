...
{
    var someField = $("textarea[data-field-name='pole1']");	// поле
    var flag = $("input[data-field-name='pole2']");	// чекбокс

    if ($(flag).is(":checked")) // проверяем, установлен ли чекбокс
    {	
		//someField.val('test1')
        //showViewElementColumn(someField);	// показываем поле
		someField.closest(".column-container").show();
    } 
    else 
    {
		//hideViewElementColumn(someField);	// скрываем поле
		//someField.val('test2')
		someField.closest(".column-container").hide();
    }
}

var HideSomeFieldView1 = function() 
{
    var someField = $("div[data-name='Поле 1']");	// поле
    var flag = $("div[data-name='Тестовый чекбокс']").find("input[type='checkbox']");	// чекбокс
...
$(document).on('change', "input[data-field-name='pole2']", function (e) { // настраиваем триггер: в документе переключается чекбокс
	console.log('вызов функции')
	console.log($("div[data-name='pole1']"))
    HideSomeFieldView(); //вызываем в документе функцию HideSomeFieldView при триггеру переключения чекбокса
});
...
scopes.onView(HideSomeFieldView1);
...
scopes.onEdit(HideSomeFieldView);
